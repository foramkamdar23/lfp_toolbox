...
%% ===================== LOAD PERCEPT LFP =====================
lfp_first_file  = "C:\Users\fkamdar\Desktop\repos\data\ok_new_cp\cpeeg02_ses02_ert1_1209.mat";
lfp_second_file = "C:\Users\fkamdar\Desktop\repos\data\ok_new_cp\cpeeg02_ses02_ert2_1226.mat";
n_lfp_channels  = 2;                 % Percept IndefiniteStreaming channels per file
align_channel   = "ONE_THREE_RIGHT";  % channel used to estimate the alignment lag

data1 = load(lfp_first_file);
data2 = load(lfp_second_file);

lfp1_all = data1.data.BrainSenseTimeDomain;
lfp2_all = data2.data.BrainSenseTimeDomain(1:2);   % first 2 structs
lfp3_all = data2.data.BrainSenseTimeDomain(3:4);   % last 2 structs
...
sig3 = lfp3_all(align_idx).TimeDomainData(:)';  
t_lfp1 = (0:numel(sig1)-1) / fs_lfp;
t_lfp2 = (0:numel(sig2)-1) / fs_lfp;
t_lfp3 = (0:numel(sig3)-1) / fs_lfp;
...
bdf_file =  "C:\Users\fkamdar\Desktop\repos\data\ok_new_cp\cpeeg02_ses02_ert_b02.bdf";
...
nan_idx = find(isnan(sig2));

if isempty(nan_idx)
    error('No NaNs found in sig2.');
end

sig3 = sig2(1 : nan_idx(1)-1);      % everything before the first NaN
sig4 = sig2(nan_idx(end)+1 : end);  % everything after the last NaN

fprintf('sig3: %d samples, sig4: %d samples (gap = %d samples)\n', ...
    numel(sig3), numel(sig4), nan_idx(end)-nan_idx(1)+1);



lfp3_env = bandpass_envelope(sig3, b_tens, a_tens);
lfp4_env = bandpass_envelope(sig4, b_tens, a_tens);

lfp2_env = nan(size(sig2));
lfp2_env(1 : nan_idx(1)-1)     = lfp3_env;
lfp2_env(nan_idx(end)+1 : end) = lfp4_env;

lag_sec1      = estimate_lag(exg7_env, lfp1_env, fs_lfp);
lag_sec2_seg3 = estimate_lag(exg7_env(1:numel(lfp3_env)), lfp3_env, fs_lfp);
lag_sec2_seg4 = estimate_lag(exg7_env(end-numel(lfp4_env)+1:end), lfp4_env, fs_lfp);

fprintf('lag2 candidates: seg3 = %.3f s, seg4 = %.3f s\n', lag_sec2_seg3, lag_sec2_seg4);

% pick whichever one is sane / contains the real TENS burst
lag_sec2 = lag_sec2_seg3;   % <-- change to _seg4 if that's the one with the real burst
...
channels_of_interest = {'ONE_THREE_LEFT','ZERO_TWO_LEFT','ONE_THREE_RIGHT','ZERO_TWO_RIGHT'};
...
labels_csv      = "C:\Users\fkamdar\Desktop\repos\lfp_toolbox\cpeeg01_session_2.csv";
...
if strlength(idcol) > 0
    ...
    trial_info_clean = table(orig_all(keep), ...
        ...
        'VariableNames', {'OrigTrial','Condition','Valence'});
...
data_feel = ft_selectdata(struct('trials', find(trial_info_clean.Condition == "FEEL")), data_clean);
data_tone = ft_selectdata(struct('trials', find(trial_info_clean.Condition == "TONE")), data_clean);
...
fprintf('FEEL=%d TONE=%d | Pos=%d Neu=%d Neg=%d\n', ...
    sum(trial_info_clean.Condition=="FEEL"), sum(trial_info_clean.Condition=="TONE"), ...
...
subject = 'CPEEG02';  session = 'ses-02';
