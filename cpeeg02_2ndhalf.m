...
%% -------------------------------------------------------------------------
% FIX: RECOMPUTE corrected_values (MISSING)
% -------------------------------------------------------------------------
...
%% -------------------------------------------------------------------------
% BLOCK 20 — CORRECT EPOCHING (0 → 1 sec) + BASELINE
% -------------------------------------------------------------------------
...
nan_mask = false(nTrials,1);

for tr = 1:nTrials
    if any(isnan(data_epoched.trial{tr}(:)))
        nan_mask(tr) = true;
    end
end
...
if isfield(data_out, 'trialinfo')
    data_out.trialinfo = data_out.trialinfo(startIdx:endIdx);
end
...
save('lfp_secondhalf.mat', 'data_out');
...
% --- 🔥 Baseline (correct variable) ---------------------------------------
cfg_bl = [];
cfg_bl.baseline = [-1 -0.5];
...
for ch = 1:nChannels
    ...
    % baseline shading
    yl = ylim;
    fill([-1 -0.5 -0.5 -1], [yl(1) yl(1) yl(2) yl(2)], ...
         [0.85 0.85 0.85], ...
...
sgtitle(sprintf('Grand average (baseline -1 to -0.5 s) | %d trials', ...
    length(clean_trials)), ...
    'FontSize', 13, 'FontWeight', 'bold');

% save('lfp_secondhalf.mat', 'data_percept_epoched');