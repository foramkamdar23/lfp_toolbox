%% Conversts UTC to PST and adds a new column to all .mat files of percept data
clear all;
clc;

folder = 'C:\Users\fkamdar\Desktop\repos\data\pdnm\pdnm091\session04';
...
for i = 1:length(files)
    ...
    for j = 1:numel(data)
        ...
        if isfield(data(j), 'IndefiniteStreaming')
            LFP_S = data(j).IndefiniteStreaming;
            fprintf('IndefiniteStreaming \n')
        elseif  isfield(data(j), 'BrainSenseTimeDomain') 
            LFP_S = data(j).BrainSenseTimeDomain;
            fprintf('BrainSenseTimeDomain \n')
        end
        
        % Skip if empty
        if isempty(LFP_S) || ~isfield(LFP_S, 'FirstPacketDateTime')
            continue;
        end
        
        % ✅ Convert once (all channels same timestamp)
        utc_str = LFP_S(1).FirstPacketDateTime;
        ...
        elseif  isfield(data(j), 'BrainSenseTimeDomain')
            for k = 1:numel(LFP_S)
                data(j).BrainSenseTimeDomain(k).FirstPacketDateTime_PDT = t_pacific;
                data(j).BrainSenseTimeDomain(k).FirstPacketDateTime_PDT_str = ...
                    datestr(t_pacific, 'yyyy-mm-dd HH:MM:SS');
            end
        end
